...
std::string config_path{"../extern/yolov3-tiny.cfg"};
std::string weights_path{"../extern/yolov3-tiny.weights"};
std::string classes_path{"../extern/coco.names"};
...
int main(int argc, char **argv)
{
    ...
    if (parser.has("input")) {
        ...
        std::unique_ptr<readwrite> capture = std::make_unique<readwrite>(input);
        cv::Mat frame;
        ...
        std::unique_ptr<readwrite> output = std::make_unique<readwrite>(image_out);
